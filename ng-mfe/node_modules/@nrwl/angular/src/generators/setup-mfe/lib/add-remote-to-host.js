...
function addRemoteToHost(host, options) {
    ...
    if (options.mfeType === 'remote' && options.host) {
        ...
        const mfRemotesNode = (0, tsquery_1.tsquery)(webpackAst, 'Identifier[name=remotes] ~ ObjectLiteralExpression', { visitAllChildren: true })[0];
        const endOfPropertiesPos = mfRemotesNode.properties.end;
        const updatedConfig = `${hostWebpackConfig.slice(0, endOfPropertiesPos)}
    \t\t"${options.appName}": 'http://localhost:${(_b = options.port) !== null && _b !== void 0 ? _b : 4200}/remoteEntry.js',${hostWebpackConfig.slice(endOfPropertiesPos)}`;
...
function addLazyLoadedRouteToHostAppModule(host, options) {
    ...
    sourceFile = (0, ast_utils_1.addRoute)(host, pathToHostAppModule, sourceFile, `{
